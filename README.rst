...
* A CKAN Extension "plugin" at ``ckanext/lightbase/plugin.py`` which, when
...
To install this package, from your CKAN virtualenv, run the following from your CKAN base folder (e.g. ``pyenv/``)::
  ...
  pip install -e git+https://github.com/okfn/ckanext-lightbase#egg=ckanext-lightbase

Then activate it by setting ``ckan.plugins = lightbase`` in your main ``ini``-file.

Orientation
===========

* Examine the source code, starting with ``ckanext/lightbase/plugin.py``
...
* These are set to point at directories within
  ``ckanext/lightbase/theme/`` (in this package).  Here we:
   * override the home page HTML ``ckanext/lightbase/theme/templates/home/index.html``
...
* The custom package edit form at ``package_form.py`` follows a deprecated
  way to make a form (using FormAlchemy). This part of the lightbase Theme needs
  updating. In the meantime, follow the instructions at: 
  http://docs.ckan.org/en/latest/forms.html

lightbase Tags With Vocabularies
==============================

To add lightbase tag vocabulary data to the database, from the ckanext-lightbase directory run:

::

    paster lightbase create-lightbase-vocabs -c <path to your ckan config file>

This data can be removed with

::

    paster lightbase clean -c <path to your ckan config file>
